...
def compute_rollout_attention(all_layer_matrices, start_layer=0):
    ...
    """
    Docstring for compute_rollout_attention

    :param all_layer_matrices: list of attention matrices from each layer, each of shape (batch_size, num_tokens, num_tokens)
    :param start_layer: index of the layer to start the rollout from
    """
    num_tokens = all_layer_matrices[0].shape[1]  # including cls token = 14*14 + 1 = 197
    ...
    # make 197 eye matrix (197x197) for each sample in the batch
    all_layer_matrices = [
    ...
    for i in range(start_layer + 1, len(all_layer_matrices)):
        joint_attention = all_layer_matrices[i].bmm(
            joint_attention
        )  # perform batch matrix multiplication
        # shape remains (batch_size, num_tokens, num_tokens) * batch_size, num_tokens, num_tokens) -> (batch_size, num_tokens, num_tokens)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # note that here the attn is per head shape is (batch_size, num_heads, num_tokens, num_tokens)
        # during backward pass, pytorch will automatically compute the gradients
        self.save_attn(attn)
        attn.register_hook(self.save_attn_gradients)
        """
        attn_gradients = ∂Loss/∂attn - the gradient of the loss with respect to the attention weights.
        This tells us how much changing each attention weight would change the output. Negative gradient means "reducing this attention weight would increase the target class score."
        """
    ...
    def relprop(self, cam, **kwargs):
        ...
        # slef.matmul2 does attn = A*V
        (cam1, cam_v) = self.matmul2.relprop(cam, **kwargs)
        """
        The output was out = attn * V. Given the relevance on the output, how much relevance should go to attn vs V?"
        This produces cam1 (relevance for attention) and cam_v (relevance for values)
        """
        cam1 /= 2
        cam_v /= 2
...
class Block(nn.Module):
    def __init__(
        ...
        # add and clone are defined in modules/layers_ours.py
        self.add1 = Add()
        self.add2 = Add()
        self.clone1 = Clone()
        self.clone2 = Clone()
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        )  # it is equivalent to linear projection after flattening patches
        # input size: (B, 3, 224, 224)
        # output size: (B, 768, 14, 14)
        # each image pathch is projected to a vector of dimension 768
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((cls_tokens, x), dim=1)  # 1, 196, 768 -> 1, 197, 768
        ...
        **kwargs,
        ...
        # our method, method name grad is legacy
            ...
            for blk in self.blocks:
                ...
                cam = grad * cam
                # cam = cam.clamp(min=0).mean(dim=0) # clamping before averaging over heads? should we average first?
                cam = cam.mean(dim=0).clamp(
                    min=0
                )  # clamping before averaging over heads? should we average first?
...
def vit_base_patch16_224(pretrained=True, num_classes=1000, checkpoint_path=None, **kwargs):
